...
const DragDrop = (() => {
  ...
  // Muestra indicador de inserción dentro de un slot
  ...
  // Calcula el índice de inserción dentro de un slot basado en coordenadas
    ...
    if (draggingFromSlot && originalSlotIndex !== null) {
      ...
      if (buildingSlot) {
        ...
        if (destSlotIndex === originalSlotIndex) {
          // Reorden DENTRO del mismo slot
          console.log('[DD] → REORDER WITHIN SLOT', originalSlotIndex, 'insertIdx:', insertIdx);
          cbs.onReorderWithinSlot?.(cid, originalSlotIndex, insertIdx);
        } else {
          // Mover a otro slot
          console.log('[DD] → MOVE BETWEEN SLOTS', originalSlotIndex, '->', destSlotIndex, 'insertIdx:', insertIdx);
